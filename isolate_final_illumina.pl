...
#Test if file is a gb file
	...
	foreach my $feat_object ( $seq_object->all_SeqFeatures) {
	    if ($feat_object->primary_tag eq "Il_Final") {
		$gprobe_start = ($feat_object->location->start)+$begin;
		$gprobe_end = ($feat_object->location->end)+$begin;
		...
		for my $tag ($feat_object->get_all_tags) {             
			...
			print "$CGI,$CGI_dist,$id,$illumina_cg[0]\n";
		    }
		} 
